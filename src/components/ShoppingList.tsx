...
export default function ShoppingList({
  ...
  function scaleAmount(amount: string, serves: number, mealServes: number): string {
  ...
  function isMealDone(meal: Meal): boolean {
    ...
    return meal.ingredients.every((_, idx) => checkedKeys.has(`${meal.id}::${idx}`));
  }
      ...
      >
        ...
        <div className="modal-card__body">
          {!hasContent ? (
            ...
            <ul className="shopping-list__meals" style={{ listStyle: "none", padding: 0, margin: 0 }}>
              {entries.map(({ meal, serves }) => {
                ...
                if (done && !expanded) {
                  return (
                    <li key={meal.id} className="shopping-meal shopping-meal--done">
                      <SwipeToDelete onDelete={() => onRemoveMeal(meal.id)}>
                        <div className="shopping-meal__inner">
                          <div className="shopping-meal__header">
                            ...
                            <div style={{ display: "flex", gap: 6 }}>
                              ...
                              <button
                                className="shopping-meal__remove mobile-hidden"
                ...
                return (
                  <li key={meal.id} className={`shopping-meal${done ? " shopping-meal--done" : ""}`}>
                    <SwipeToDelete onDelete={() => onRemoveMeal(meal.id)}>
                      <div className="shopping-meal__inner">
                        <div className="shopping-meal__header">
                        ...
                        <div className="serves-row">
                          ...
                          <button
                            ...
                            onClick={() => onUpdateServes(meal.id, Math.max(1, serves - 1))}
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              if (!isNaN(val) && val > 0) onUpdateServes(meal.id, val);
                            }}
                          />
                          <button
                            className="serves-row__step-btn"
                        ...
                        <ul className="shopping-ingredients">
                          {meal.ingredients.map((ingredient, idx) => {
                            ...
                            const scaled = scaleAmount(ingredient.amount, serves, meal.serves);
              ...
              {adHocItems.map((item) => {
                ...
                return (
                  <li
                    ...
                    className={`shopping-ingredient${isBought ? " shopping-ingredient--bought" : ""}`}
                  >
                    <SwipeToDelete onDelete={() => onRemoveAdHoc(item.id)}>
                      <div className="shopping-adhoc__inner">
                        <input
                          type="checkbox"
                          className="shopping-ingredient__check"
                          checked={isBought}
                        ...
                        <button
                          className="shopping-adhoc-remove mobile-hidden"
        ...
        <div className="modal-card__footer">
          {confirmClear ? (
            <>
              <span className="shopping-clear-confirm__label">{T.confirmDelete}</span>
              <button
